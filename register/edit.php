...
//Databaseに格納
  ...
  if ($link) {
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $newfile=$_POST['filecontents'];
        if ($_FILES['image']['size']>0) { //添付ファイルが存在する時に実行
        ...
        if ($page == "1") {
          ...
          $sql="UPDATE trip_season SET name_sei='".$name_sei."',name_mei='".$name_mei."',name_sei_kana='".$name_sei_kana."',name_mei_kana='".$name_mei_kana."',
          ...
          email='".$email."',filecontents='".$newfile."',season_sequence='".$sequence."' WHERE id='".$id."';";
        } else {
          $sql="UPDATE trip_ranking SET name_sei='".$name_sei."',name_mei='".$name_mei."',name_sei_kana='".$name_sei_kana."',name_mei_kana='".$name_mei_kana."',
          season='".$season."',year='".$year."',month='".$month."',day='".$day."',category='".$category."',subject='".$subject."',
          zip='".$zip."',pref='".$pref."',city='".$city."',house_number='".$house_number."',building='".$building."',tel='".$tel."',memo='".$memo."',
          email='".$email."',filecontents='".$newfile."',ranking='".$sequence."' WHERE id='".$id."';";
        }
      ...
      if($result=mysqli_query($link,$sql)){
        foreach($result as $row) {
          ...
          if ($page == "1") {
            $sequence =  $row['season_sequence'];
          } else {
            $sequence =  $row['ranking'];
          }
        }
        //値を読み取った後に表示する
        include("table.php");
